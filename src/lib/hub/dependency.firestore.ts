...
// src/lib/hub/dependency.firestore.ts
//
// Ported from EngineXDraw's apps/web/src/lib/hub/dependency.firestore.ts,
// itself ported from CivilOS Hub's lib/firestore/dependency.firestore.ts
// — see the header comment on event.firestore.ts for the db()/
// firestorePaths wiring changes.
//
// bumpModuleVersion() is the function every future write path in this app
// (Phase 6 outgoing sync, and any earlier phase that writes something the
// rest of the ecosystem should be able to depend on) is expected to call
// after a Firestore write — it handles version increment, event
// emission, and the approval-downgrade cascade together so callers don't
// have to remember all three separately.
...
export async function bumpModuleVersion(projectId: string, moduleId: ModuleId): Promise<number> {
  const ref = doc(db(), firestorePaths.hubModuleVersion(projectId, moduleId));
