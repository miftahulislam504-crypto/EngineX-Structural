...
export function CombinedFootingPanel({ onAddElement, onDeleteElement }: CombinedFootingPanelProps) {
  ...
  return (
    <div className="space-y-4">
      <div>
        <h3 className="text-sm font-medium text-slate-200 mb-2">Combined Footing</h3>

        {combinedFootings.length === 0 ? (
          <p className="text-xs text-slate-500">কোনো Combined Footing যোগ করা হয়নি।</p>
        ) : (
      ...
      {noMaterials ? (
        ...
        <form onSubmit={handleSubmit} className="space-y-2.5 border-t border-slate-800 pt-3">
          ...
          <div>
            <p className="text-xs text-slate-500 mb-1">Column A — কেন্দ্রবিন্দু (m)</p>
          ...
          <div>
            <p className="text-xs text-slate-500 mb-1">Column B — কেন্দ্রবিন্দু (m)</p>
            <div className="grid grid-cols-3 gap-1.5">
              <input
                type="number"
                step="any"
