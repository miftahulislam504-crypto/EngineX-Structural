...
export function FootingPanel({ onAddElement, onDeleteElement }: FootingPanelProps) {
  ...
  return (
    <div className="space-y-4">
      <div>
        <h3 className="text-sm font-medium text-slate-200 mb-2">Footing</h3>

        {footings.length === 0 ? (
          <p className="text-xs text-slate-500">কোনো Footing যোগ করা হয়নি।</p>
        ) : (
      ...
      {noMaterials ? (
        ...
        <form onSubmit={handleSubmit} className="space-y-2.5 border-t border-slate-800 pt-3">
          ...
          <div>
            <p className="text-xs text-slate-500 mb-1">Location — কেন্দ্রবিন্দু (m)</p>
